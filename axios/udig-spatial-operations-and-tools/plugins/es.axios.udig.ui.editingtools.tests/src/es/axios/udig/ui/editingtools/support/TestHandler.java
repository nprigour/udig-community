...
import org.locationtech.udig.project.internal.MapFactory;
...
public class TestHandler extends EditToolHandler {
	...
	public TestHandler(int numFeatures, String featureTypeName) throws Exception {
		...
		setContext(context);
		testing = true;
		bb = new TestEditBlackboard(10, 10, context.worldToScreenTransform(), map.getLayersInternal().get(0)
					.layerToMapTransform());
	...
	public SimpleFeature getFeature(final int i) throws IOException {
		FeatureSource<SimpleFeatureType, SimpleFeature> source = map.getLayersInternal().get(0).getResource(
